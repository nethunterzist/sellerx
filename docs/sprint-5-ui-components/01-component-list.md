...
| auth | login-form, register-form, email-verification-banner, forgot-password-form, reset-password-form | Giriş, kayıt, e-posta doğrulama, parola sıfırlama formları. |
| dashboard | chart-view, cities-view, dashboard-chart, dashboard-filters, dashboard-tabs, period-cards, pl-view, trends-view, order-detail-panel, product-detail-panel, stock-depletion-banner, chart-metric-selector, fifo-profitability-section | Dashboard grafik, filtre, dönem kartları, PL görünümü, metrik seçici, FIFO karlılık. |
...
| returns | claims-table, return-summary-cards, return-trend-chart, return-reasons-chart, return-cost-breakdown, top-returned-products | İade/claim listesi, grafikler. |
...
| providers | theme-provider, error-boundary, keyboard-shortcuts-provider, chunk-error-handler, websocket-provider, query-provider | Tema, hata sınırı, kısayol, chunk hata yönetimi, WebSocket, query provider. |
| calculator | calculator-cta, calculator-form, calculator-results, calculator-summary | Kar hesaplama formu, sonuçlar, özet. |
| charts | lazy-chart | Lazy-loaded grafik bileşeni. |
| customer-analytics | customer-detail-panel, product-repeat-detail-panel | Müşteri analitik detay panelleri. |
| shared | (ortak bileşenler) | Paylaşılan yardımcı bileşenler. |
| (root) | store-sync-progress | Mağaza sync ilerleme (tek dosya). |

**Not:** `ui/` klasöründe yeni eklenen: `date-range-picker.tsx`.

**Toplam:** 27 klasör + root'ta 1 bileşen. Bileşen sayısı: ~200+ .tsx dosyası (ui dahil).
