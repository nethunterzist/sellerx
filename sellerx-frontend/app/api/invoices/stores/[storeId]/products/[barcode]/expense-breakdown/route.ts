...
/**
 * GET /api/invoices/stores/[storeId]/products/[barcode]/expense-breakdown
 *
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/invoices/stores/${storeId}/products/${encodeURIComponent(decodedBarcode)}/expense-breakdown${queryString ? `?${queryString}` : ""}`,
      {
        method: "GET",
        headers,
      }
    ...
    if (isDev) console.error("[API] GET /invoices/stores/[storeId]/products/[barcode]/expense-breakdown error:", error);
    return NextResponse.json(
