...
public interface SubscriptionRepository extends JpaRepository<Subscription, UUID> {
    ...
    @Query("SELECT s.plan.code AS planCode, COUNT(s) AS subscriptionCount FROM Subscription s WHERE s.status IN ('ACTIVE', 'TRIAL') GROUP BY s.plan.code")
    List<PlanCountProjection> countSubscriptionsByPlan();
