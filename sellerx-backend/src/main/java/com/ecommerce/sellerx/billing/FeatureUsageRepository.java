...
public interface FeatureUsageRepository extends JpaRepository<FeatureUsage, UUID> {
    ...
    @Query("SELECT fu.featureCode AS featureCode, fu.usageCount AS usageCount FROM FeatureUsage fu WHERE fu.user.id = :userId AND fu.periodStart <= :now AND fu.periodEnd >= :now")
    List<FeatureUsageSummaryProjection> findCurrentUsageSummary(@Param("userId") Long userId, @Param("now") LocalDateTime now);
}
